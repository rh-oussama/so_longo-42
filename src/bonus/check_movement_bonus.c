...
/*   Updated: 2024/02/21 21:23:19 by oussama          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	backtrack(char ***map_visited, int x, int y, t_back_track **player)
{
	...
	map_visited[0][y][x] == 'X' || map_visited[1][y][x] == 'T')
		return ;
...
int	check_map_dimension(char **map, t_mapinfo info)
{
	...
	{
		if ((int)ft_strlen(map[info.y]) != info.x_max)
			return (1);
		info.x = 0;
		while (info.x < info.x_max)
		{
			...
			|| info.x == info.x_max - 1) && info.current_char != '1')
				...
				info.exit++;
			else if (!(info.current_char == 'C' || info.current_char == 'X' || \
